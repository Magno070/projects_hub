...
class ViewModelProvider<T extends ChangeNotifier> extends StatefulWidget {
  ...
  const ViewModelProvider({
    ...
    this.create,
    this.value,
  }) : assert(create != null || value != null, 'Either create or value must be provided');
  ...
  static T of<T extends ChangeNotifier>(BuildContext context) {
    final provider = context.dependOnInheritedWidgetOfExactType<_InheritedViewModelProvider<T>>();
    if (provider == null) {
      throw FlutterError('ViewModelProvider.of<$T> called with a context that does not contain a ViewModelProvider<$T>');
    }
...
class _ViewModelProviderState<T extends ChangeNotifier> extends State<ViewModelProvider<T>> {
  late T _viewModel;
...
class _InheritedViewModelProvider<T extends ChangeNotifier> extends InheritedWidget {
  final T viewModel;
